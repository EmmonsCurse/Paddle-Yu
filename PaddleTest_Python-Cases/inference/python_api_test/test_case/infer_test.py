...
class InferenceTest(object):
    ...
    def mkldnn_test(
        ...
        # self.pd_config.set_cpu_math_library_num_threads(4)

        print("Use GPU is: {}".format(self.pd_config.use_gpu()))
        print("Use IR_OPTIM is: {}".format(self.pd_config.ir_optim()))

        self.pd_config.set_mkldnn_cache_capacity(mkldnn_cache_capacity)
        print("Use MKLDNN is: {}".format(self.pd_config.mkldnn_enabled()))
        print("Use MKLDNN_INT8 is: {}".format(self.pd_config.mkldnn_int8_enabled()))
        predictor = paddle_infer.create_predictor(self.pd_config)
        ...
        for i, output_data_name in enumerate(output_names):
            ...
            start = time.time()
            print("Time before diff compare", start)
            for j, out_data in enumerate(output_data):
                ...
                print("The result of diff:", diff)
            end = time.time()
            print("Time after diff compare", end)
            print("The calculating time of diff:", end - start)
    ...
    def onnxruntime_test(self, input_data_dict: dict, output_data_dict: dict, repeat=2, delta=1e-5):
        ...
        print("Use GPU is: {}".format(self.pd_config.use_gpu()))
        print("Use IR_OPTIM is: {}".format(self.pd_config.ir_optim()))
        print("Use ONNXRuntime is: {}".format(self.pd_config.onnxruntime_enabled()))
        predictor = paddle_infer.create_predictor(self.pd_config)
    ...
    def trt_more_bz_test(
        ...
        for i in range(repeat):
            predictor.run()
        if tuned:  # collect_shape_range_info 收集动态 shape 需要 predictor 后再退出
            return 0
        output_names = predictor.get_output_names()
        for i, output_data_name in enumerate(output_names):
            output_handle = predictor.get_output_handle(output_data_name)
