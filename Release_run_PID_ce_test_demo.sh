...
echo "demo 3: advanced_share_external_data_resnet50:"
...
echo "demo 4: advanced_multi_thread_resnet50:"
...
echo "demo 5: advanced_custom_operator_resnet50:"
...
echo "demo 6.3: resnet50_trt_fp32:"
# 使用 Trt Fp32 运行样例
...
echo "demo 6.4: resnet50_trt_fp16:"
# 使用 Trt Fp16 运行样例
...
echo "demo 6.5.1: resnet50_trt_int8_generate_calib:"
# 使用 Trt Int8 运行样例
...
echo "demo 6.5.2: resnet50_trt_int8:"
python infer_resnet.py --model_file=../../cpu/resnet50/resnet50/inference.pdmodel --params_file=../../cpu/resnet50/resnet50/inference.pdiparams --run_mode=trt_int8
count_error

echo "demo 6.6: resnet50_try_dynamic shape_trt_fp32:"
...
echo "demo 7.4.1: yolov3_trt_int8_generate_calib:"
# 使用 Trt Int8 运行样例
...
echo "demo 7.4.2: yolov3_trt_int8:"
python infer_yolov3.py --model_file=../../cpu/yolov3/yolov3_r50vd_dcn_270e_coco/model.pdmodel --params_file=../../cpu/yolov3/yolov3_r50vd_dcn_270e_coco/model.pdiparams --run_mode=trt_int8
count_error

echo "demo 7.5: yolov3_try_dynamic_shape_trt_fp32:"
# 使用 Try dynamic shape 运行样例 -error
...
echo "demo 8.1: resnet50_tuned_dynamic_shape_tune:"
...
echo "demo 8.2: resnet50_tuned_dynamic_shape:"
...
echo "demo 10: mixed_mask_detection:"
# demo 10: 口罩检测预测样例
demo=`expr ${demo} + 1`
cd ../mask_detection/
...
echo "demo 3.2: resnet50_trt_fp32:"
...
echo "demo 3.3: resnet50_trt_fp16:"
...
echo "demo 3.4.1: resnet50_trt_int8_generate_calib:"
...
echo "demo 3.4.2: resnet50_trt_int8:"
# 加载校准表执行预测
...
echo "demo 3.5: resnet50_quant_trt_int8:"
...
echo "demo 3.6: resnet50_try_dynamic_shape_trt_fp32:"
...
echo "demo 4.4.1: yolov3_trt_int8_generate_calib:"
...
echo "demo 4.4.2: yolov3_trt_int8:"
# 加载校准表执行预测
./build/yolov3_test --model_file yolov3_r50vd_dcn_270e_coco/model.pdmodel --params_file yolov3_r50vd_dcn_270e_coco/model.pdiparams --run_mode=trt_int8
count_error

echo "demo 4.5: yolov3_try_dynamic_shape_trt_fp32:"
# 使用 TensorRT dynamic shape 运行样例（以 Fp32 为例）-error
...
echo "demo 5.1.1: tuned_dynamic_shape_ResNet50_Vd_离线tune测试"
# ResNet50_Vd
# 离线tune测试
...
echo "demo 5.1.2: tuned_dynamic_shape_ResNet50_Vd_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.1.3: tuned_dynamic_shape_ResNet50_Vd_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/clas --model_file inference_model/ResNet50_vd/inference.pdmodel  --params_file inference_model/ResNet50_vd/inference.pdiparams --hs="224:448" --ws="224:448" --no_seen_hs="112:672" --no_seen_ws="112:672" --tuned_dynamic_shape --serialize
count_error


echo "demo 5.2.1: tuned_dynamic_shape_GhostNet_x1_0_离线tune测试"
# GhostNet_x1_0
# 离线tune测试
...
echo "demo 5.2.2: tuned_dynamic_shape_GhostNet_x1_0_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.2.3: tuned_dynamic_shape_GhostNet_x1_0_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/clas --model_file inference_model/GhostNet_x1_0/inference.pdmodel --params_file inference_model/GhostNet_x1_0/inference.pdiparams --hs="224:448" --ws="224:448" --no_seen_hs="112:672" --no_seen_ws="112:672" --tuned_dynamic_shape --serialize
count_error


echo "demo 5.3.1: tuned_dynamic_shape_DenseNet121_离线tune测试"
# DenseNet121
# 离线tune测试
...
echo "demo 5.3.2: tuned_dynamic_shape_DenseNet121_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.3.3: tuned_dynamic_shape_DenseNet121_动态shape及序列化测试"
# 动态shape及反序列化测试
./build/clas --model_file inference_model/DenseNet121/inference.pdmodel --params_file inference_model/DenseNet121/inference.pdiparams --hs="224:448" --ws="224:448" --no_seen_hs="112:672" --no_seen_ws="112:672" --tuned_dynamic_shape --serialize
count_error


echo "demo 5.4.1: tuned_dynamic_shape_HRNet_W32_C_离线tune测试"
# HRNet_W32_C
# 离线tune测试
...
echo "demo 5.4.2: tuned_dynamic_shape_HRNet_W32_C_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.4.3: tuned_dynamic_shape_HRNet_W32_C_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/clas --model_file inference_model/HRNet_W32_C/inference.pdmodel --params_file inference_model/HRNet_W32_C/inference.pdiparams --hs="224" --ws="224" --no_seen_hs="448" --no_seen_ws="448" --tuned_dynamic_shape --serialize
count_error


echo "demo 5.5.1: tuned_dynamic_shape_InceptionV4_离线tune测试"
# InceptionV4
# 离线tune测试
...
echo "demo 5.5.2: tuned_dynamic_shape_InceptionV4_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.5.3: tuned_dynamic_shape_InceptionV4_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/clas --model_file inference_model/InceptionV4/inference.pdmodel --params_file inference_model/InceptionV4/inference.pdiparams --hs="224" --ws="224" --no_seen_hs="448" --no_seen_ws="448" --tuned_dynamic_shape --serialize
count_error


echo "demo 5.6.1: tuned_dynamic_shape_ViT_base_patch16_224_离线tune测试"
# ViT_base_patch16_224
...
echo "demo 5.6.2: tuned_dynamic_shape_ViT_base_patch16_224_动态shape及序列化测试"
...
echo "demo 5.6.3: tuned_dynamic_shape_ViT_base_patch16_224_动态shape及反序列化测试"
# 动态shape及反序列化测试（注意：目前需要config加配置：config->Exp_DisableTensorRtOPs({"elementwise_add"});）
./build/clas --model_file inference_model/ViT_base_patch16_224/inference.pdmodel --params_file inference_model/ViT_base_patch16_224/inference.pdiparams --hs="224" --ws="224"--no_seen_hs="224" --no_seen_ws="224" --tuned_dynamic_shape --serialize
count_error


echo "demo 5.7.1: tuned_dynamic_shape_DeiT_base_patch16_224_离线tune测试"
# DeiT_base_patch16_224
# 离线tune测试
...
echo "demo 5.7.2: tuned_dynamic_shape_DeiT_base_patch16_224_动态shape及序列化测试"
# 动态shape及序列化测试（注意：目前需要config加配置：config->Exp_DisableTensorRtOPs({"elementwise_add"});）
...
echo "demo 5.7.3: tuned_dynamic_shape_DeiT_base_patch16_224_动态shape及反序列化测试"
# 动态shape及反序列化测试（注意：目前需要config加配置：config->Exp_DisableTensorRtOPs({"elementwise_add"});）
./build/clas --model_file inference_model/DeiT_base_patch16_224/inference.pdmodel --params_file inference_model/DeiT_base_patch16_224/inference.pdiparams --hs="224" --ws="224"--no_seen_hs="224" --no_seen_ws="224" --tuned_dynamic_shape --serialize
count_error
...
echo "demo 5.8.1: tuned_dynamic_shape_yolov3_darknet53_270e_coco_离线tune测试"
# yolov3_darknet53_270e_coco
# 离线tune测试
...
echo "demo 5.8.2: tuned_dynamic_shape_yolov3_darknet53_270e_coco_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.8.3: tuned_dynamic_shape_yolov3_darknet53_270e_coco_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/detect --model_file inference_model/yolov3_darknet53_270e_coco/model.pdmodel --params_file inference_model/yolov3_darknet53_270e_coco/model.pdiparams --hs="608" --ws="608" --no_seen_hs="416" --no_seen_ws="416" --tuned_dynamic_shape --serialize
count_error
...
echo "demo 5.10.3: tuned_dynamic_shape_faster_rcnn_r50_fpn_1x_coco_动态shape及反序列化测试"
...
echo "demo 5.11.3: tuned_dynamic_shape_mask_rcnn_r50_vd_fpn_2x_coco_动态shape及反序列化测试"
# 动态shape及反序列化测试（注意：目前需要config加配置：config->Exp_DisableTensorRtOPs({"roi_align", "elementwise_add"});）
./build/detect --model_file inference_model/mask_rcnn_r50_vd_fpn_2x_coco/model.pdmodel --params_file inference_model/mask_rcnn_r50_vd_fpn_2x_coco/model.pdiparams --hs="608" --ws="608" --no_seen_hs="416" --no_seen_ws="416" --tuned_dynamic_shape --serialize
count_error
...
echo "demo 5.12.1: tuned_dynamic_shape_ch_ppocr_server_v2.0_det_infer_离线tune测试"
# 离线tune测试
...
echo "demo 5.12.2: tuned_dynamic_shape_ch_ppocr_server_v2.0_det_infer_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.12.3: tuned_dynamic_shape_ch_ppocr_server_v2.0_det_infer_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/ocr_det --model_file ocr_inference_model/ch_ppocr_server_v2.0_det_infer/inference.pdmodel --params_file ocr_inference_model/ch_ppocr_server_v2.0_det_infer/inference.pdiparams --hs="640" --ws="640" --no_seen_hs="320" --no_seen_ws="320" --tuned_dynamic_shape --serialize
count_error
...
echo "demo 5.13.1: tuned_dynamic_shape_ch_ppocr_mobile_v2.0_cls_infer_离线tune测试"
# 离线tune测试
...
echo "demo 5.13.2: tuned_dynamic_shape_ch_ppocr_mobile_v2.0_cls_infer_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.13.3: tuned_dynamic_shape_ch_ppocr_mobile_v2.0_cls_infer_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/ocr_cls --model_file ocr_inference_model/ch_ppocr_mobile_v2.0_cls_infer/inference.pdmodel --params_file ocr_inference_model/ch_ppocr_mobile_v2.0_cls_infer/inference.pdiparams --hs="640" --ws="640" --no_seen_hs="320" --no_seen_ws="320" --tuned_dynamic_shape --serialize
count_error
...
echo "demo 5.14.1: tuned_dynamic_shape_ch_ppocr_mobile_v2.0_rec_infer_离线tune测试"
# 离线tune测试
...
echo "demo 5.14.2: tuned_dynamic_shape_ch_ppocr_mobile_v2.0_rec_infer_动态shape及序列化测试"
# 动态shape及序列化测试
...
echo "demo 5.14.3: tuned_dynamic_shape_ch_ppocr_mobile_v2.0_rec_infer_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/ocr_rec --model_file ocr_inference_model/ch_ppocr_server_v2.0_rec_infer/inference.pdmodel --params_file ocr_inference_model/ch_ppocr_server_v2.0_rec_infer/inference.pdiparams --hs="32" --ws="32" --no_seen_hs="32" --no_seen_ws="320" --tuned_dynamic_shape --serialize
count_error
...
echo "demo 5.15.1: tuned_dynamic_shape_ernie_model_4_离线tune测试"
# 离线tune测试
./build/ernie --model_dir nlp_inference_model/ernie_model_4 --tune
count_error


echo "demo 5.15.2: tuned_dynamic_shape_ernie_model_4_动态shape及序列化测试"
# 动态shape及序列化测试
./build/ernie --model_dir nlp_inference_model/ernie_model_4 --tuned_dynamic_shape --serialize
count_error

echo "demo 5.15.3: tuned_dynamic_shape_ernie_model_4_动态shape及反序列化测试"
# 动态shape及反序列化测试
./build/ernie --model_dir nlp_inference_model/ernie_model_4 --tuned_dynamic_shape --serialize
count_error
...
echo "demo 11: advanced-share_external_data"
# demo 11 advanced-share_external_data
demo=`expr ${demo} + 1`

# 运行 share_external_data 预测样例
# 运行 CPU 样例
echo "demo 11.1: advanced_share_external_data_cpu"
cd ../share_external_data/ && sed -i "s/TensorRT-7.1.3.4/TensorRT-8.0.3.4/" compile.sh && sh run.sh
count_error

echo "demo 11.2: advanced_share_external_data_gpu"
# 运行 GPU 样例
./build/resnet50_share_data --model_file resnet50/inference.pdmodel --params_file resnet50/inference.pdiparams --use_gpu=1
count_error


echo "demo 12: advanced_multi_thread"
# demo 12 advanced-multi_thread
demo=`expr ${demo} + 1`
...
echo "demo 13: advanced_x86_gru_int8"
# demo 13 advanced_x86_gru_int8
demo=`expr ${demo} + 1`
cd ../x86_gru_int8/
...
echo "demo 14: mixed_LIC2020"
# demo 14 cpp_mixed_LIC2020
demo=`expr ${demo} + 1`

# 运行 C++ LIC2020 关系抽取 demo
cd ../../mixed/LIC2020/
...
echo "demo 15: mixed_internal"
# demo 15 cpp_mixed_internal
demo=`expr ${demo} + 1`
